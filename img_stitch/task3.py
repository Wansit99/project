...
def morph_erode(img):
    ...
    tmp = np.pad(erode_img, (1, 1), 'edge')
    for y in range(1, H + 1):
        for x in range(1, W + 1):
            if np.sum(MF * tmp[y - 1:y + 2, x - 1:x + 2]) >= 255:
                erode_img[y - 1, x - 1] = 255
...
def morph_dilate(img):
    ...
    tmp = np.pad(dilate_img, (1, 1), 'edge')
    # erode
    for y in range(1, H + 1):
        for x in range(1, W + 1):
            if np.sum(MF * tmp[y - 1:y + 2, x - 1:x + 2]) < 255 * 4:
                dilate_img[y - 1, x - 1] = 0
...
def morph_open(img):
    ...
    # TO DO: implement your solution here
    out = morph_dilate(img)
    open_img = morph_erode(out)
    return open_img
...
def denoise(img):
    ...
    denoise_img = 255 - denoise_img
    denoise_img = morph_open(denoise_img)
    denoise_img = 255 - denoise_img
    return denoise_img
...
def boundary(img):
    ...
    # TO DO: implement your solution here
    erode_out = morph_erode(img)
    erode_out = morph_erode(erode_out)
    dilate_out = morph_dilate(img)
    dilate_out = morph_dilate(dilate_out)
    bound_img = np.abs(erode_out - dilate_out) * 255
...
if __name__ == "__main__":
    ...
    # cv2.imwrite('results/task3_denoise.jpg', denoise_img)
    denoise_img = cv2.imread('results/task3_denoise.jpg', cv2.IMREAD_GRAYSCALE)
    bound_img = boundary(denoise_img)
    cv2.imwrite('results/task3_boundary.jpg', bound_img)
